...
float Error(CvScalar f1, CvScalar f2, CvScalar f3, CvScalar f4, CvScalar F)
{
	CvScalar f;
...
int main()
{
	...
	maskMemory = (int**)malloc(sizeof(int*) * (h));

	for (int i = 0; i < h; i++)
		maskMemory[i] = (int*)malloc(sizeof(int) * w);
		...
		for (int y = 0; y < h; y++)
			for (int x = 0; x < w; x++)
			{
				if (maskMemory[y][x] ==1)
				{
					...
					if(cnt %100 ==0) cvSet2D(src, y, x, F);
					colorMemory[y][x] = F;
				
					//sumF += Error(f1,f2,f3,f4,F);
